...
<h2>Test cases/Possible errors</h2>
...
Test if webpage is responsive when changed to mobile web version |Load webpage using a mobile phone and observe if there are any significant changes| Webpage is resized to suit mobile web users | Most of the webpage is resized properly,with a few images and input boxes that are too big that had to be resized accordingly. However for the main page when it is switched to the mobile version, the navigation bar is blocking some of the content in the carousel slider. For the mobile web layout, for the local storage page, due to the positioning of the button, the navigation bar is slightly bugged which cannot be fixed.|Fail|  
